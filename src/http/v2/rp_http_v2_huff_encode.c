...
typedef struct {
    uint32_t  code;
    uint32_t  len;
} rp_http_v2_huff_encode_code_t;


static rp_http_v2_huff_encode_code_t  rp_http_v2_huff_encode_table[256] =
{
...
static rp_http_v2_huff_encode_code_t  rp_http_v2_huff_encode_table_lc[256] =
{
    {0x00001ff8, 13}, {0x007fffd8, 23}, {0x0fffffe2, 28}, {0x0fffffe3, 28},
    {0x0fffffe4, 28}, {0x0fffffe5, 28}, {0x0fffffe6, 28}, {0x0fffffe7, 28},
    {0x0fffffe8, 28}, {0x00ffffea, 24}, {0x3ffffffc, 30}, {0x0fffffe9, 28},
...
#if (RP_HAVE_GCC_BSWAP64)
#define rp_http_v2_huff_encode_buf(dst, buf)                                 \
...
size_t
rp_http_v2_huff_encode(u_char *src, size_t len, u_char *dst, rp_uint_t lower)
{
    u_char                          *end;
    size_t                           hlen;
    rp_uint_t                       buf, pending, code;
    rp_http_v2_huff_encode_code_t  *table, *next;

    table = lower ? rp_http_v2_huff_encode_table_lc
                  : rp_http_v2_huff_encode_table;
    hlen = 0;
    buf = 0;
    pending = 0;
    ...
    while (src != end) {
        ...
        rp_http_v2_huff_encode_buf(&dst[hlen], buf);
    ...
    buf |= (rp_uint_t) -1 >> pending;

    pending = rp_align(pending, 8);

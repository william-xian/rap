...
typedef struct {
    uint32_t  code;
    uint32_t  len;
} rap_http_v2_huff_encode_code_t;


static rap_http_v2_huff_encode_code_t  rap_http_v2_huff_encode_table[256] =
{
...
static rap_http_v2_huff_encode_code_t  rap_http_v2_huff_encode_table_lc[256] =
{
    {0x00001ff8, 13}, {0x007fffd8, 23}, {0x0fffffe2, 28}, {0x0fffffe3, 28},
    {0x0fffffe4, 28}, {0x0fffffe5, 28}, {0x0fffffe6, 28}, {0x0fffffe7, 28},
    {0x0fffffe8, 28}, {0x00ffffea, 24}, {0x3ffffffc, 30}, {0x0fffffe9, 28},
...
#if (RAP_HAVE_GCC_BSWAP64)
#define rap_http_v2_huff_encode_buf(dst, buf)                                 \
...
size_t
rap_http_v2_huff_encode(u_char *src, size_t len, u_char *dst, rap_uint_t lower)
{
    u_char                          *end;
    size_t                           hlen;
    rap_uint_t                       buf, pending, code;
    rap_http_v2_huff_encode_code_t  *table, *next;

    table = lower ? rap_http_v2_huff_encode_table_lc
                  : rap_http_v2_huff_encode_table;
    hlen = 0;
    buf = 0;
    pending = 0;
    ...
    while (src != end) {
        ...
        rap_http_v2_huff_encode_buf(&dst[hlen], buf);
    ...
    buf |= (rap_uint_t) -1 >> pending;

    pending = rap_align(pending, 8);

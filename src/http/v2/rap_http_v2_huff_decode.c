...
typedef struct {
    ...
    u_char  sym;
    u_char  ending;
} rap_http_v2_huff_decode_code_t;


static rap_inline rap_int_t rap_http_v2_huff_decode_bits(u_char *state,
    u_char *ending, rap_uint_t bits, u_char **dst);


static rap_http_v2_huff_decode_code_t  rap_http_v2_huff_decode_codes[256][16] =
{
    /* 0 */
    {
...
rap_int_t
rap_http_v2_huff_decode(u_char *state, u_char *src, size_t len, u_char **dst,
    rap_uint_t last, rap_log_t *log)
{
    ...
    while (src != end) {
        ...
        if (rap_http_v2_huff_decode_bits(state, &ending, ch >> 4, dst)
            != RAP_OK)
        {
            rap_log_debug2(RAP_LOG_DEBUG_HTTP, log, 0,
        ...
        if (rap_http_v2_huff_decode_bits(state, &ending, ch & 0xf, dst)
            != RAP_OK)
        {
            rap_log_debug2(RAP_LOG_DEBUG_HTTP, log, 0,
                           "http2 huffman decoding error at state %d: "
...
rap_http_v2_huff_decode_bits(u_char *state, u_char *ending, rap_uint_t bits,
    u_char **dst)
{
    rap_http_v2_huff_decode_code_t  code;

    code = rap_http_v2_huff_decode_codes[*state][bits];

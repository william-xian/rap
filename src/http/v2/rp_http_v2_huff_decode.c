...
typedef struct {
    ...
    u_char  sym;
    u_char  ending;
} rp_http_v2_huff_decode_code_t;


static rp_inline rp_int_t rp_http_v2_huff_decode_bits(u_char *state,
    u_char *ending, rp_uint_t bits, u_char **dst);


static rp_http_v2_huff_decode_code_t  rp_http_v2_huff_decode_codes[256][16] =
{
    /* 0 */
    {
...
rp_int_t
rp_http_v2_huff_decode(u_char *state, u_char *src, size_t len, u_char **dst,
    rp_uint_t last, rp_log_t *log)
{
    ...
    while (src != end) {
        ...
        if (rp_http_v2_huff_decode_bits(state, &ending, ch >> 4, dst)
            != RP_OK)
        {
            rp_log_debug2(RP_LOG_DEBUG_HTTP, log, 0,
        ...
        if (rp_http_v2_huff_decode_bits(state, &ending, ch & 0xf, dst)
            != RP_OK)
        {
            rp_log_debug2(RP_LOG_DEBUG_HTTP, log, 0,
                           "http2 huffman decoding error at state %d: "
...
rp_http_v2_huff_decode_bits(u_char *state, u_char *ending, rp_uint_t bits,
    u_char **dst)
{
    rp_http_v2_huff_decode_code_t  code;

    code = rp_http_v2_huff_decode_codes[*state][bits];

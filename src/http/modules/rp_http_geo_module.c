...
typedef struct {
    rp_http_geo_range_t           **low;
    rp_http_variable_value_t       *default_value;
} rp_http_geo_high_ranges_t;
...
typedef struct {
    ...
    rp_http_variable_value_t       *value;
    size_t                           offset;
} rp_http_geo_variable_value_node_t;


typedef struct {
    rp_http_variable_value_t       *value;
    rp_str_t                       *net;
    rp_http_geo_high_ranges_t       high;
    rp_radix_tree_t                *tree;
#if (RP_HAVE_INET6)
    rp_radix_tree_t                *tree6;
#endif
...
} rp_http_geo_conf_ctx_t;


typedef struct {
    union {
        rp_http_geo_trees_t         trees;
        rp_http_geo_high_ranges_t   high;
    } u;
...
static rp_int_t rp_http_geo_addr(rp_http_request_t *r,
    rp_http_geo_ctx_t *ctx, rp_addr_t *addr);
static rp_int_t rp_http_geo_real_addr(rp_http_request_t *r,
    rp_http_geo_ctx_t *ctx, rp_addr_t *addr);
static char *rp_http_geo_block(rp_conf_t *cf, rp_command_t *cmd, void *conf);
static char *rp_http_geo(rp_conf_t *cf, rp_command_t *dummy, void *conf);
static char *rp_http_geo_range(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *value);
static char *rp_http_geo_add_range(rp_conf_t *cf,
    rp_http_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end);
static rp_uint_t rp_http_geo_delete_range(rp_conf_t *cf,
    rp_http_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end);
static char *rp_http_geo_cidr(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *value);
static char *rp_http_geo_cidr_add(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_cidr_t *cidr, rp_str_t *value, rp_str_t *net);
static rp_http_variable_value_t *rp_http_geo_value(rp_conf_t *cf,
    rp_http_geo_conf_ctx_t *ctx, rp_str_t *value);
static char *rp_http_geo_add_proxy(rp_conf_t *cf,
    rp_http_geo_conf_ctx_t *ctx, rp_cidr_t *cidr);
static rp_int_t rp_http_geo_cidr_value(rp_conf_t *cf, rp_str_t *net,
    rp_cidr_t *cidr);
static char *rp_http_geo_include(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *name);
static rp_int_t rp_http_geo_include_binary_base(rp_conf_t *cf,
    rp_http_geo_conf_ctx_t *ctx, rp_str_t *name);
static void rp_http_geo_create_binary_base(rp_http_geo_conf_ctx_t *ctx);
static u_char *rp_http_geo_copy_values(u_char *base, u_char *p,
    rp_rbtree_node_t *node, rp_rbtree_node_t *sentinel);


static rp_command_t  rp_http_geo_commands[] = {

    { rp_string("geo"),
      RP_HTTP_MAIN_CONF|RP_CONF_BLOCK|RP_CONF_TAKE12,
      rp_http_geo_block,
...
static rp_http_module_t  rp_http_geo_module_ctx = {
...
rp_module_t  rp_http_geo_module = {
    RP_MODULE_V1,
    &rp_http_geo_module_ctx,              /* module context */
    rp_http_geo_commands,                 /* module directives */
...
typedef struct {
    ...
    uint32_t  endianness;
    uint32_t  crc32;
} rp_http_geo_header_t;


static rp_http_geo_header_t  rp_http_geo_header = {
...
static rp_int_t
rp_http_geo_cidr_variable(rp_http_request_t *r, rp_http_variable_value_t *v,
    uintptr_t data)
{
    rp_http_geo_ctx_t *ctx = (rp_http_geo_ctx_t *) data;
    ...
    if (rp_http_geo_addr(r, ctx, &addr) != RP_OK) {
        vv = (rp_http_variable_value_t *)
...
static rp_int_t
rp_http_geo_range_variable(rp_http_request_t *r, rp_http_variable_value_t *v,
    uintptr_t data)
{
    rp_http_geo_ctx_t *ctx = (rp_http_geo_ctx_t *) data;
...
static rp_int_t
rp_http_geo_addr(rp_http_request_t *r, rp_http_geo_ctx_t *ctx,
    rp_addr_t *addr)
{
    rp_array_t  *xfwd;

    if (rp_http_geo_real_addr(r, ctx, addr) != RP_OK) {
        return RP_ERROR;
    }
...
static rp_int_t
rp_http_geo_real_addr(rp_http_request_t *r, rp_http_geo_ctx_t *ctx,
    rp_addr_t *addr)
{
    ...
    v = rp_http_get_flushed_variable(r, ctx->index);
    ...
    if (rp_parse_addr(r->pool, addr, v->data, v->len) == RP_OK) {
        return RP_OK;
    }
...
static char *
rp_http_geo_block(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    char                     *rv;
    size_t                    len;
    rp_str_t                *value, name;
    ...
    rp_http_geo_ctx_t       *geo;
    rp_http_geo_conf_ctx_t   ctx;
#if (RP_HAVE_INET6)
    ...
    geo = rp_palloc(cf->pool, sizeof(rp_http_geo_ctx_t));
    if (geo == NULL) {
    ...
    if (cf->args->nelts == 3) {
        ...
        geo->index = rp_http_get_variable_index(cf, &name);
    ...
    rp_memzero(&ctx, sizeof(rp_http_geo_conf_ctx_t));
    ...
    ctx.pool = cf->pool;
    ctx.data_size = sizeof(rp_http_geo_header_t)
                  + sizeof(rp_http_variable_value_t)
                  + 0x10000 * sizeof(rp_http_geo_range_t *);
    ctx.allow_binary_include = 1;

    save = *cf;
    cf->pool = pool;
    cf->ctx = &ctx;
    cf->handler = rp_http_geo;
    ...
    if (ctx.ranges) {
        ...
        if (ctx.high.low && !ctx.binary_include) {
            for (i = 0; i < 0x10000; i++) {
                ...
                len = a->nelts * sizeof(rp_http_geo_range_t);
            ...
            if (ctx.allow_binary_include
                ...
                && ctx.includes == 1)
            {
                rp_http_geo_create_binary_base(&ctx);
            }
        }

        if (ctx.high.default_value == NULL) {
            ctx.high.default_value = &rp_http_variable_null_value;
        }

        geo->u.high = ctx.high;

        var->get_handler = rp_http_geo_range_variable;
        ...
        var->get_handler = rp_http_geo_cidr_variable;
        var->data = (uintptr_t) geo;

        if (rp_radix32tree_insert(ctx.tree, 0, 0,
                                   (uintptr_t) &rp_http_variable_null_value)
            == RP_ERROR)
        {
            goto failed;
        }
...
static char *
rp_http_geo(rp_conf_t *cf, rp_command_t *dummy, void *conf)
{
    char                     *rv;
    rp_str_t                *value;
    rp_cidr_t                cidr;
    ...
    if (cf->args->nelts == 1) {
        ...
        else if (rp_strcmp(value[0].data, "proxy_recursive") == 0) {
    ...
    if (rp_strcmp(value[0].data, "include") == 0) {
        ...
        rv = rp_http_geo_include(cf, ctx, &value[1]);

        goto done;

    } else if (rp_strcmp(value[0].data, "proxy") == 0) {

        if (rp_http_geo_cidr_value(cf, &value[1], &cidr) != RP_OK) {
            goto failed;
        }

        rv = rp_http_geo_add_proxy(cf, ctx, &cidr);

        goto done;
    }

    if (ctx->ranges) {
        rv = rp_http_geo_range(cf, ctx, value);

    } else {
        rv = rp_http_geo_cidr(cf, ctx, value);
    }
...
static char *
rp_http_geo_range(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *value)
{
    u_char      *p, *last;
    in_addr_t    start, end;
    ...
    if (rp_strcmp(value[0].data, "default") == 0) {
        ...
        ctx->high.default_value = rp_http_geo_value(cf, ctx, &value[1]);
        if (ctx->high.default_value == NULL) {
            return RP_CONF_ERROR;
        }
    ...
    if (del) {
        if (rp_http_geo_delete_range(cf, ctx, start, end)) {
            rp_conf_log_error(RP_LOG_WARN, cf, 0,
    ...
    ctx->value = rp_http_geo_value(cf, ctx, &value[1]);
    ...
    return rp_http_geo_add_range(cf, ctx, start, end);
...
static char *
rp_http_geo_add_range(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        if (a == NULL) {
            a = rp_array_create(ctx->temp_pool, 64,
                                 sizeof(rp_http_geo_range_t));
            if (a == NULL) {
                return RP_CONF_ERROR;
            }

            ctx->high.low[h] = (rp_http_geo_range_t *) a;
        }
        ...
        while (i) {
            ...
            if (s > (rp_uint_t) range[i].end) {
                ...
                rp_memmove(&range[i + 2], &range[i + 1],
                            (a->nelts - 2 - i) * sizeof(rp_http_geo_range_t));
                ...
                && e < (rp_uint_t) range[i].end)
            {
                ...
                rp_memmove(&range[i + 3], &range[i + 1],
                            (a->nelts - 3 - i) * sizeof(rp_http_geo_range_t));
                ...
                && e < (rp_uint_t) range[i].end)
            {
                ...
                rp_memmove(&range[i + 1], &range[i],
                            (a->nelts - 1 - i) * sizeof(rp_http_geo_range_t));
        ...
        rp_memmove(&range[1], &range[0],
                    (a->nelts - 1) * sizeof(rp_http_geo_range_t));
...
static rp_uint_t
rp_http_geo_delete_range(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    in_addr_t start, in_addr_t end)
{
    in_addr_t              n;
...
static char *
rp_http_geo_cidr(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *value)
{
    char        *rv;
    rp_int_t    rc, del;
    ...
    if (rp_strcmp(value[0].data, "default") == 0) {
        ...
        rv = rp_http_geo_cidr_add(cf, ctx, &cidr, &value[1], &value[0]);
    ...
    if (rp_http_geo_cidr_value(cf, net, &cidr) != RP_OK) {
        return RP_CONF_ERROR;
    }
    ...
    return rp_http_geo_cidr_add(cf, ctx, &cidr, &value[1], net);
}


static char *
rp_http_geo_cidr_add(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_cidr_t *cidr, rp_str_t *value, rp_str_t *net)
{
    rp_int_t                   rc;
    rp_http_variable_value_t  *val, *old;

    val = rp_http_geo_value(cf, ctx, value);
...
static rp_http_variable_value_t *
rp_http_geo_value(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *value)
{
    uint32_t                             hash;
    rp_http_variable_value_t           *val;
    rp_http_geo_variable_value_node_t  *gvvn;

    hash = rp_crc32_long(value->data, value->len);

    gvvn = (rp_http_geo_variable_value_node_t *)
    ...
    val = rp_palloc(ctx->pool, sizeof(rp_http_variable_value_t));
    if (val == NULL) {
        return NULL;
    }
    ...
    gvvn = rp_palloc(ctx->temp_pool,
                      sizeof(rp_http_geo_variable_value_node_t));
    if (gvvn == NULL) {
        return NULL;
    }
    ...
    ctx->data_size += rp_align(sizeof(rp_http_variable_value_t) + value->len,
                                sizeof(void *));
...
static char *
rp_http_geo_add_proxy(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_cidr_t *cidr)
{
    rp_cidr_t  *c;

    if (ctx->proxies == NULL) {
        ctx->proxies = rp_array_create(ctx->pool, 4, sizeof(rp_cidr_t));
        if (ctx->proxies == NULL) {
            return RP_CONF_ERROR;
        }
    }

    c = rp_array_push(ctx->proxies);
    if (c == NULL) {
        return RP_CONF_ERROR;
    }
...
static rp_int_t
rp_http_geo_cidr_value(rp_conf_t *cf, rp_str_t *net, rp_cidr_t *cidr)
{
    rp_int_t  rc;
...
static char *
rp_http_geo_include(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *name)
{
    char       *rv;
    rp_str_t   file;
    ...
    if (ctx->ranges) {
        ...
        switch (rp_http_geo_include_binary_base(cf, ctx, &file)) {
        case RP_OK:
            return RP_CONF_OK;
        case RP_ERROR:
            return RP_CONF_ERROR;
...
static rp_int_t
rp_http_geo_include_binary_base(rp_conf_t *cf, rp_http_geo_conf_ctx_t *ctx,
    rp_str_t *name)
{
    u_char                     *base, ch;
    time_t                      mtime;
    ...
    rp_file_t                  file;
    rp_file_info_t             fi;
    rp_http_geo_range_t       *range, **ranges;
    rp_http_geo_header_t      *header;
    rp_http_variable_value_t  *vv;
    ...
    header = (rp_http_geo_header_t *) base;

    if (size < 16 || rp_memcmp(&rp_http_geo_header, header, 12) != 0) {
        rp_conf_log_error(RP_LOG_WARN, cf, 0,
    ...
    vv = (rp_http_variable_value_t *) (base + sizeof(rp_http_geo_header_t));

    while (vv->data) {
        len = rp_align(sizeof(rp_http_variable_value_t) + vv->len,
                        sizeof(void *));
        rp_crc32_update(&crc32, (u_char *) vv, len);
        vv->data += (size_t) base;
        vv = (rp_http_variable_value_t *) ((u_char *) vv + len);
    }
    rp_crc32_update(&crc32, (u_char *) vv, sizeof(rp_http_variable_value_t));
    vv++;

    ranges = (rp_http_geo_range_t **) vv;

    for (i = 0; i < 0x10000; i++) {
        rp_crc32_update(&crc32, (u_char *) &ranges[i], sizeof(void *));
        if (ranges[i]) {
            ranges[i] = (rp_http_geo_range_t *)
                            ((u_char *) ranges[i] + (size_t) base);
        }
    }

    range = (rp_http_geo_range_t *) &ranges[0x10000];

    while ((u_char *) range < base + size) {
        while (range->value) {
            rp_crc32_update(&crc32, (u_char *) range,
                             sizeof(rp_http_geo_range_t));
            range->value = (rp_http_variable_value_t *)
                               ((u_char *) range->value + (size_t) base);
            range++;
        }
        rp_crc32_update(&crc32, (u_char *) range, sizeof(void *));
        range = (rp_http_geo_range_t *) ((u_char *) range + sizeof(void *));
    }
...
static void
rp_http_geo_create_binary_base(rp_http_geo_conf_ctx_t *ctx)
{
    u_char                              *p;
    uint32_t                             hash;
    rp_str_t                            s;
    ...
    rp_http_geo_range_t                *r, *range, **ranges;
    rp_http_geo_header_t               *header;
    rp_http_geo_variable_value_node_t  *gvvn;
    ...
    p = rp_cpymem(fm.addr, &rp_http_geo_header,
                   sizeof(rp_http_geo_header_t));

    p = rp_http_geo_copy_values(fm.addr, p, ctx->rbtree.root,
                                 ctx->rbtree.sentinel);

    p += sizeof(rp_http_variable_value_t);

    ranges = (rp_http_geo_range_t **) p;

    p += 0x10000 * sizeof(rp_http_geo_range_t *);
    ...
    for (i = 0; i < 0x10000; i++) {
        ...
        range = (rp_http_geo_range_t *) p;
        ranges[i] = (rp_http_geo_range_t *) (p - (u_char *) fm.addr);
        ...
        do {
            ...
            range->value = (rp_http_variable_value_t *) gvvn->offset;
            range->start = r->start;
            range->end = r->end;
            range++;
    ...
    header->crc32 = rp_crc32_long((u_char *) fm.addr
                                       + sizeof(rp_http_geo_header_t),
                                   fm.size - sizeof(rp_http_geo_header_t));
...
static u_char *
rp_http_geo_copy_values(u_char *base, u_char *p, rp_rbtree_node_t *node,
    rp_rbtree_node_t *sentinel)
{
    rp_http_variable_value_t           *vv;
    rp_http_geo_variable_value_node_t  *gvvn;
    ...
    gvvn = (rp_http_geo_variable_value_node_t *) node;
    gvvn->offset = p - base;

    vv = (rp_http_variable_value_t *) p;
    *vv = *gvvn->value;
    p += sizeof(rp_http_variable_value_t);
    ...
    p = rp_http_geo_copy_values(base, p, node->left, sentinel);

    return rp_http_geo_copy_values(base, p, node->right, sentinel);
}


/*
 * Copyright (C) Igor Sysoev
 * Copyright (C) Rap, Inc.
 */


#include <rap_config.h>
#include <rap_core.h>
#include <rap_http.h>


typedef struct {
    rap_chain_t         *free;
    rap_chain_t         *busy;
} rap_http_chunked_filter_ctx_t;


static rap_int_t rap_http_chunked_filter_init(rap_conf_t *cf);
static rap_chain_t *rap_http_chunked_create_trailers(rap_http_request_t *r,
    rap_http_chunked_filter_ctx_t *ctx);


static rap_http_module_t  rap_http_chunked_filter_module_ctx = {
    NULL,                                  /* preconfiguration */
    rap_http_chunked_filter_init,          /* postconfiguration */

    NULL,                                  /* create main configuration */
    NULL,                                  /* init main configuration */

    NULL,                                  /* create server configuration */
    NULL,                                  /* merge server configuration */

    NULL,                                  /* create location configuration */
    NULL                                   /* merge location configuration */
};


rap_module_t  rap_http_chunked_filter_module = {
    RAP_MODULE_V1,
    &rap_http_chunked_filter_module_ctx,   /* module context */
    NULL,                                  /* module directives */
    RAP_HTTP_MODULE,                       /* module type */
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    NULL,                                  /* exit process */
    NULL,                                  /* exit master */
    RAP_MODULE_V1_PADDING
};


static rap_http_output_header_filter_pt  rap_http_next_header_filter;
static rap_http_output_body_filter_pt    rap_http_next_body_filter;


static rap_int_t
rap_http_chunked_header_filter(rap_http_request_t *r)
{
    rap_http_core_loc_conf_t       *clcf;
    rap_http_chunked_filter_ctx_t  *ctx;

    if (r->headers_out.status == RAP_HTTP_NOT_MODIFIED
        || r->headers_out.status == RAP_HTTP_NO_CONTENT
        || r->headers_out.status < RAP_HTTP_OK
        || r != r->main
        || r->method == RAP_HTTP_HEAD)
    {
        return rap_http_next_header_filter(r);
    }

    if (r->headers_out.content_length_n == -1
        || r->expect_trailers)
    {
        clcf = rap_http_get_module_loc_conf(r, rap_http_core_module);

        if (r->http_version >= RAP_HTTP_VERSION_11
            && clcf->chunked_transfer_encoding)
        {
            if (r->expect_trailers) {
                rap_http_clear_content_length(r);
            }

            r->chunked = 1;

            ctx = rap_pcalloc(r->pool, sizeof(rap_http_chunked_filter_ctx_t));
            if (ctx == NULL) {
                return RAP_ERROR;
            }

            rap_http_set_ctx(r, ctx, rap_http_chunked_filter_module);

        } else if (r->headers_out.content_length_n == -1) {
            r->keepalive = 0;
        }
    }

    return rap_http_next_header_filter(r);
}


static rap_int_t
rap_http_chunked_body_filter(rap_http_request_t *r, rap_chain_t *in)
{
    u_char                         *chunk;
    off_t                           size;
    rap_int_t                       rc;
    rap_buf_t                      *b;
    rap_chain_t                    *out, *cl, *tl, **ll;
    rap_http_chunked_filter_ctx_t  *ctx;

    if (in == NULL || !r->chunked || r->header_only) {
        return rap_http_next_body_filter(r, in);
    }

    ctx = rap_http_get_module_ctx(r, rap_http_chunked_filter_module);

    out = NULL;
    ll = &out;

    size = 0;
    cl = in;

    for ( ;; ) {
        rap_log_debug1(RAP_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "http chunk: %O", rap_buf_size(cl->buf));

        size += rap_buf_size(cl->buf);

        if (cl->buf->flush
            || cl->buf->sync
            || rap_buf_in_memory(cl->buf)
            || cl->buf->in_file)
        {
            tl = rap_alloc_chain_link(r->pool);
            if (tl == NULL) {
                return RAP_ERROR;
            }

            tl->buf = cl->buf;
            *ll = tl;
            ll = &tl->next;
        }

        if (cl->next == NULL) {
            break;
        }

        cl = cl->next;
    }

    if (size) {
        tl = rap_chain_get_free_buf(r->pool, &ctx->free);
        if (tl == NULL) {
            return RAP_ERROR;
        }

        b = tl->buf;
        chunk = b->start;

        if (chunk == NULL) {
            /* the "0000000000000000" is 64-bit hexadecimal string */

            chunk = rap_palloc(r->pool, sizeof("0000000000000000" CRLF) - 1);
            if (chunk == NULL) {
                return RAP_ERROR;
            }

            b->start = chunk;
            b->end = chunk + sizeof("0000000000000000" CRLF) - 1;
        }

        b->tag = (rap_buf_tag_t) &rap_http_chunked_filter_module;
        b->memory = 0;
        b->temporary = 1;
        b->pos = chunk;
        b->last = rap_sprintf(chunk, "%xO" CRLF, size);

        tl->next = out;
        out = tl;
    }

    if (cl->buf->last_buf) {
        tl = rap_http_chunked_create_trailers(r, ctx);
        if (tl == NULL) {
            return RAP_ERROR;
        }

        cl->buf->last_buf = 0;

        *ll = tl;

        if (size == 0) {
            tl->buf->pos += 2;
        }

    } else if (size > 0) {
        tl = rap_chain_get_free_buf(r->pool, &ctx->free);
        if (tl == NULL) {
            return RAP_ERROR;
        }

        b = tl->buf;

        b->tag = (rap_buf_tag_t) &rap_http_chunked_filter_module;
        b->temporary = 0;
        b->memory = 1;
        b->pos = (u_char *) CRLF;
        b->last = b->pos + 2;

        *ll = tl;

    } else {
        *ll = NULL;
    }

    rc = rap_http_next_body_filter(r, out);

    rap_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &out,
                            (rap_buf_tag_t) &rap_http_chunked_filter_module);

    return rc;
}


static rap_chain_t *
rap_http_chunked_create_trailers(rap_http_request_t *r,
    rap_http_chunked_filter_ctx_t *ctx)
{
    size_t            len;
    rap_buf_t        *b;
    rap_uint_t        i;
    rap_chain_t      *cl;
    rap_list_part_t  *part;
    rap_table_elt_t  *header;

    len = 0;

    part = &r->headers_out.trailers.part;
    header = part->elts;

    for (i = 0; /* void */; i++) {

        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
            }

            part = part->next;
            header = part->elts;
            i = 0;
        }

        if (header[i].hash == 0) {
            continue;
        }

        len += header[i].key.len + sizeof(": ") - 1
               + header[i].value.len + sizeof(CRLF) - 1;
    }

    cl = rap_chain_get_free_buf(r->pool, &ctx->free);
    if (cl == NULL) {
        return NULL;
    }

    b = cl->buf;

    b->tag = (rap_buf_tag_t) &rap_http_chunked_filter_module;
    b->temporary = 0;
    b->memory = 1;
    b->last_buf = 1;

    if (len == 0) {
        b->pos = (u_char *) CRLF "0" CRLF CRLF;
        b->last = b->pos + sizeof(CRLF "0" CRLF CRLF) - 1;
        return cl;
    }

    len += sizeof(CRLF "0" CRLF CRLF) - 1;

    b->pos = rap_palloc(r->pool, len);
    if (b->pos == NULL) {
        return NULL;
    }

    b->last = b->pos;

    *b->last++ = CR; *b->last++ = LF;
    *b->last++ = '0';
    *b->last++ = CR; *b->last++ = LF;

    part = &r->headers_out.trailers.part;
    header = part->elts;

    for (i = 0; /* void */; i++) {

        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
            }

            part = part->next;
            header = part->elts;
            i = 0;
        }

        if (header[i].hash == 0) {
            continue;
        }

        rap_log_debug2(RAP_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "http trailer: \"%V: %V\"",
                       &header[i].key, &header[i].value);

        b->last = rap_copy(b->last, header[i].key.data, header[i].key.len);
        *b->last++ = ':'; *b->last++ = ' ';

        b->last = rap_copy(b->last, header[i].value.data, header[i].value.len);
        *b->last++ = CR; *b->last++ = LF;
    }

    *b->last++ = CR; *b->last++ = LF;

    return cl;
}


static rap_int_t
rap_http_chunked_filter_init(rap_conf_t *cf)
{
    rap_http_next_header_filter = rap_http_top_header_filter;
    rap_http_top_header_filter = rap_http_chunked_header_filter;

    rap_http_next_body_filter = rap_http_top_body_filter;
    rap_http_top_body_filter = rap_http_chunked_body_filter;

    return RAP_OK;
}

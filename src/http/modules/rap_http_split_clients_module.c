...
} rap_http_split_clients_part_t;


typedef struct {
    rap_http_complex_value_t    value;
    rap_array_t                 parts;
} rap_http_split_clients_ctx_t;


static char *rap_conf_split_clients_block(rap_conf_t *cf, rap_command_t *cmd,
    void *conf);
static char *rap_http_split_clients(rap_conf_t *cf, rap_command_t *dummy,
    void *conf);

static rap_command_t  rap_http_split_clients_commands[] = {
...
static rap_http_module_t  rap_http_split_clients_module_ctx = {
...
rap_module_t  rap_http_split_clients_module = {
    RAP_MODULE_V1,
    &rap_http_split_clients_module_ctx,    /* module context */
    rap_http_split_clients_commands,       /* module directives */
...
static rap_int_t
rap_http_split_clients_variable(rap_http_request_t *r,
    rap_http_variable_value_t *v, uintptr_t data)
{
    rap_http_split_clients_ctx_t *ctx = (rap_http_split_clients_ctx_t *) data;

    uint32_t                        hash;
    rap_str_t                       val;
    rap_uint_t                      i;
    rap_http_split_clients_part_t  *part;

    *v = rap_http_variable_null_value;

    if (rap_http_complex_value(r, &ctx->value, &val) != RAP_OK) {
        return RAP_OK;
    }
...
rap_conf_split_clients_block(rap_conf_t *cf, rap_command_t *cmd, void *conf)
{
    ...
    rap_http_split_clients_ctx_t        *ctx;
    rap_http_split_clients_part_t       *part;
    rap_http_compile_complex_value_t     ccv;

    ctx = rap_pcalloc(cf->pool, sizeof(rap_http_split_clients_ctx_t));
    if (ctx == NULL) {
    ...
    var->get_handler = rap_http_split_clients_variable;
    var->data = (uintptr_t) ctx;

    if (rap_array_init(&ctx->parts, cf->pool, 2,
                       sizeof(rap_http_split_clients_part_t))
        != RAP_OK)
    {
        return RAP_CONF_ERROR;
    }
    ...
    save = *cf;
    cf->ctx = ctx;
    cf->handler = rap_http_split_clients;
...
static char *
rap_http_split_clients(rap_conf_t *cf, rap_command_t *dummy, void *conf)
{
    rap_int_t                       n;
    rap_str_t                      *value;
    rap_http_split_clients_ctx_t   *ctx;

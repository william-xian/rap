...
} rp_http_split_clients_part_t;


typedef struct {
    rp_http_complex_value_t    value;
    rp_array_t                 parts;
} rp_http_split_clients_ctx_t;


static char *rp_conf_split_clients_block(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
static char *rp_http_split_clients(rp_conf_t *cf, rp_command_t *dummy,
    void *conf);

static rp_command_t  rp_http_split_clients_commands[] = {
...
static rp_http_module_t  rp_http_split_clients_module_ctx = {
...
rp_module_t  rp_http_split_clients_module = {
    RP_MODULE_V1,
    &rp_http_split_clients_module_ctx,    /* module context */
    rp_http_split_clients_commands,       /* module directives */
...
static rp_int_t
rp_http_split_clients_variable(rp_http_request_t *r,
    rp_http_variable_value_t *v, uintptr_t data)
{
    rp_http_split_clients_ctx_t *ctx = (rp_http_split_clients_ctx_t *) data;

    uint32_t                        hash;
    rp_str_t                       val;
    rp_uint_t                      i;
    rp_http_split_clients_part_t  *part;

    *v = rp_http_variable_null_value;

    if (rp_http_complex_value(r, &ctx->value, &val) != RP_OK) {
        return RP_OK;
    }
...
rp_conf_split_clients_block(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    ...
    rp_http_split_clients_ctx_t        *ctx;
    rp_http_split_clients_part_t       *part;
    rp_http_compile_complex_value_t     ccv;

    ctx = rp_pcalloc(cf->pool, sizeof(rp_http_split_clients_ctx_t));
    if (ctx == NULL) {
    ...
    var->get_handler = rp_http_split_clients_variable;
    var->data = (uintptr_t) ctx;

    if (rp_array_init(&ctx->parts, cf->pool, 2,
                       sizeof(rp_http_split_clients_part_t))
        != RP_OK)
    {
        return RP_CONF_ERROR;
    }
    ...
    save = *cf;
    cf->ctx = ctx;
    cf->handler = rp_http_split_clients;
...
static char *
rp_http_split_clients(rp_conf_t *cf, rp_command_t *dummy, void *conf)
{
    rp_int_t                       n;
    rp_str_t                      *value;
    rp_http_split_clients_ctx_t   *ctx;

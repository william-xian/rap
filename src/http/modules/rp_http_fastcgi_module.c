...
typedef struct {
    ...
    rp_http_fastcgi_params_t      params;
#if (RP_HTTP_CACHE)
    rp_http_fastcgi_params_t      params_cache;
#endif

    rp_array_t                   *params_source;
    rp_array_t                   *catch_stderr;

    rp_array_t                   *fastcgi_lengths;
    rp_array_t                   *fastcgi_values;
...
#if (RP_PCRE)
    rp_regex_t                   *split_regex;
    rp_str_t                      split_name;
#endif
} rp_http_fastcgi_loc_conf_t;


typedef enum {
    rp_http_fastcgi_st_version = 0,
    rp_http_fastcgi_st_type,
    rp_http_fastcgi_st_request_id_hi,
    rp_http_fastcgi_st_request_id_lo,
    rp_http_fastcgi_st_content_length_hi,
    rp_http_fastcgi_st_content_length_lo,
    rp_http_fastcgi_st_padding_length,
    rp_http_fastcgi_st_reserved,
    rp_http_fastcgi_st_data,
    rp_http_fastcgi_st_padding
} rp_http_fastcgi_state_e;


typedef struct {
    u_char                        *start;
    u_char                        *end;
} rp_http_fastcgi_split_part_t;


typedef struct {
    rp_http_fastcgi_state_e       state;
    u_char                        *pos;
    u_char                        *last;
    rp_uint_t                     type;
    size_t                         length;
    ...
    rp_array_t                   *split_parts;

    rp_str_t                      script_name;
    rp_str_t                      path_info;
} rp_http_fastcgi_ctx_t;


#define RP_HTTP_FASTCGI_RESPONDER      1

#define RP_HTTP_FASTCGI_KEEP_CONN      1

#define RP_HTTP_FASTCGI_BEGIN_REQUEST  1
#define RP_HTTP_FASTCGI_ABORT_REQUEST  2
#define RP_HTTP_FASTCGI_END_REQUEST    3
#define RP_HTTP_FASTCGI_PARAMS         4
#define RP_HTTP_FASTCGI_STDIN          5
#define RP_HTTP_FASTCGI_STDOUT         6
#define RP_HTTP_FASTCGI_STDERR         7
#define RP_HTTP_FASTCGI_DATA           8
...
typedef struct {
    ...
    u_char  flags;
    u_char  reserved[5];
} rp_http_fastcgi_begin_request_t;
...
typedef struct {
    ...
    u_char  request_id_hi;
    u_char  request_id_lo;
} rp_http_fastcgi_header_small_t;


typedef struct {
    rp_http_fastcgi_header_t         h0;
    rp_http_fastcgi_begin_request_t  br;
    rp_http_fastcgi_header_small_t   h1;
} rp_http_fastcgi_request_start_t;


static rp_int_t rp_http_fastcgi_eval(rp_http_request_t *r,
    rp_http_fastcgi_loc_conf_t *flcf);
#if (RP_HTTP_CACHE)
static rp_int_t rp_http_fastcgi_create_key(rp_http_request_t *r);
#endif
static rp_int_t rp_http_fastcgi_create_request(rp_http_request_t *r);
static rp_int_t rp_http_fastcgi_reinit_request(rp_http_request_t *r);
static rp_int_t rp_http_fastcgi_body_output_filter(void *data,
    rp_chain_t *in);
static rp_int_t rp_http_fastcgi_process_header(rp_http_request_t *r);
static rp_int_t rp_http_fastcgi_input_filter_init(void *data);
static rp_int_t rp_http_fastcgi_input_filter(rp_event_pipe_t *p,
    rp_buf_t *buf);
static rp_int_t rp_http_fastcgi_non_buffered_filter(void *data,
    ssize_t bytes);
static rp_int_t rp_http_fastcgi_process_record(rp_http_request_t *r,
    rp_http_fastcgi_ctx_t *f);
static void rp_http_fastcgi_abort_request(rp_http_request_t *r);
static void rp_http_fastcgi_finalize_request(rp_http_request_t *r,
    rp_int_t rc);

static rp_int_t rp_http_fastcgi_add_variables(rp_conf_t *cf);
static void *rp_http_fastcgi_create_main_conf(rp_conf_t *cf);
static void *rp_http_fastcgi_create_loc_conf(rp_conf_t *cf);
static char *rp_http_fastcgi_merge_loc_conf(rp_conf_t *cf,
    void *parent, void *child);
static rp_int_t rp_http_fastcgi_init_params(rp_conf_t *cf,
    rp_http_fastcgi_loc_conf_t *conf, rp_http_fastcgi_params_t *params,
    rp_keyval_t *default_params);

static rp_int_t rp_http_fastcgi_script_name_variable(rp_http_request_t *r,
    rp_http_variable_value_t *v, uintptr_t data);
static rp_int_t rp_http_fastcgi_path_info_variable(rp_http_request_t *r,
    rp_http_variable_value_t *v, uintptr_t data);
static rp_http_fastcgi_ctx_t *rp_http_fastcgi_split(rp_http_request_t *r,
    rp_http_fastcgi_loc_conf_t *flcf);

static char *rp_http_fastcgi_pass(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
static char *rp_http_fastcgi_split_path_info(rp_conf_t *cf,
    rp_command_t *cmd, void *conf);
static char *rp_http_fastcgi_store(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
#if (RP_HTTP_CACHE)
static char *rp_http_fastcgi_cache(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
static char *rp_http_fastcgi_cache_key(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
#endif

static char *rp_http_fastcgi_lowat_check(rp_conf_t *cf, void *post,
    void *data);


static rp_conf_post_t  rp_http_fastcgi_lowat_post =
    { rp_http_fastcgi_lowat_check };


static rp_conf_bitmask_t  rp_http_fastcgi_next_upstream_masks[] = {
...
rp_module_t  rp_http_fastcgi_module;


static rp_command_t  rp_http_fastcgi_commands[] = {

    { rp_string("fastcgi_pass"),
      RP_HTTP_LOC_CONF|RP_HTTP_LIF_CONF|RP_CONF_TAKE1,
      rp_http_fastcgi_pass,
      RP_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { rp_string("fastcgi_index"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_str_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, index),
      NULL },

    { rp_string("fastcgi_split_path_info"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_http_fastcgi_split_path_info,
      RP_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { rp_string("fastcgi_store"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_http_fastcgi_store,
      RP_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { rp_string("fastcgi_store_access"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE123,
      rp_conf_set_access_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.store_access),
      NULL },

    { rp_string("fastcgi_buffering"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.buffering),
      NULL },

    { rp_string("fastcgi_request_buffering"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.request_buffering),
      NULL },

    { rp_string("fastcgi_ignore_client_abort"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.ignore_client_abort),
      NULL },

    { rp_string("fastcgi_bind"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE12,
      rp_http_upstream_bind_set_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.local),
      NULL },

    { rp_string("fastcgi_socket_keepalive"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.socket_keepalive),
      NULL },

    { rp_string("fastcgi_connect_timeout"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.connect_timeout),
      NULL },

    { rp_string("fastcgi_send_timeout"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.send_timeout),
      NULL },

    { rp_string("fastcgi_send_lowat"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.send_lowat),
      &rp_http_fastcgi_lowat_post },

    { rp_string("fastcgi_buffer_size"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.buffer_size),
      NULL },

    { rp_string("fastcgi_pass_request_headers"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.pass_request_headers),
      NULL },

    { rp_string("fastcgi_pass_request_body"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.pass_request_body),
      NULL },

    { rp_string("fastcgi_intercept_errors"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.intercept_errors),
      NULL },

    { rp_string("fastcgi_read_timeout"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.read_timeout),
      NULL },

    { rp_string("fastcgi_buffers"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE2,
      rp_conf_set_bufs_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.bufs),
      NULL },

    { rp_string("fastcgi_busy_buffers_size"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.busy_buffers_size_conf),
      NULL },

    { rp_string("fastcgi_force_ranges"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.force_ranges),
      NULL },

    { rp_string("fastcgi_limit_rate"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.limit_rate),
      NULL },

#if (RP_HTTP_CACHE)

    { rp_string("fastcgi_cache"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_http_fastcgi_cache,
      RP_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { rp_string("fastcgi_cache_key"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_http_fastcgi_cache_key,
      RP_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { rp_string("fastcgi_cache_path"),
      RP_HTTP_MAIN_CONF|RP_CONF_2MORE,
      rp_http_file_cache_set_slot,
      RP_HTTP_MAIN_CONF_OFFSET,
      offsetof(rp_http_fastcgi_main_conf_t, caches),
      &rp_http_fastcgi_module },

    { rp_string("fastcgi_cache_bypass"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_http_set_predicate_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_bypass),
      NULL },

    { rp_string("fastcgi_no_cache"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_http_set_predicate_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.no_cache),
      NULL },

    { rp_string("fastcgi_cache_valid"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_http_file_cache_valid_set_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_valid),
      NULL },

    { rp_string("fastcgi_cache_min_uses"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_num_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_min_uses),
      NULL },

    { rp_string("fastcgi_cache_max_range_offset"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_off_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_max_range_offset),
      NULL },

    { rp_string("fastcgi_cache_use_stale"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_conf_set_bitmask_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_use_stale),
      &rp_http_fastcgi_next_upstream_masks },

    { rp_string("fastcgi_cache_methods"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_conf_set_bitmask_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_methods),
      &rp_http_upstream_cache_method_mask },

    { rp_string("fastcgi_cache_lock"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_lock),
      NULL },

    { rp_string("fastcgi_cache_lock_timeout"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_lock_timeout),
      NULL },

    { rp_string("fastcgi_cache_lock_age"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_lock_age),
      NULL },

    { rp_string("fastcgi_cache_revalidate"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_revalidate),
      NULL },

    { rp_string("fastcgi_cache_background_update"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.cache_background_update),
      NULL },

#endif

    { rp_string("fastcgi_temp_path"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1234,
      rp_conf_set_path_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.temp_path),
      NULL },

    { rp_string("fastcgi_max_temp_file_size"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.max_temp_file_size_conf),
      NULL },

    { rp_string("fastcgi_temp_file_write_size"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_size_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.temp_file_write_size_conf),
      NULL },

    { rp_string("fastcgi_next_upstream"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_conf_set_bitmask_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.next_upstream),
      &rp_http_fastcgi_next_upstream_masks },

    { rp_string("fastcgi_next_upstream_tries"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_num_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.next_upstream_tries),
      NULL },

    { rp_string("fastcgi_next_upstream_timeout"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_msec_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.next_upstream_timeout),
      NULL },

    { rp_string("fastcgi_param"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE23,
      rp_http_upstream_param_set_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, params_source),
      NULL },

    { rp_string("fastcgi_pass_header"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_str_array_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.pass_headers),
      NULL },

    { rp_string("fastcgi_hide_header"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_str_array_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.hide_headers),
      NULL },

    { rp_string("fastcgi_ignore_headers"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_1MORE,
      rp_conf_set_bitmask_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, upstream.ignore_headers),
      &rp_http_upstream_ignore_headers_masks },

    { rp_string("fastcgi_catch_stderr"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_TAKE1,
      rp_conf_set_str_array_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, catch_stderr),
      NULL },

    { rp_string("fastcgi_keep_conn"),
      RP_HTTP_MAIN_CONF|RP_HTTP_SRV_CONF|RP_HTTP_LOC_CONF|RP_CONF_FLAG,
      rp_conf_set_flag_slot,
      RP_HTTP_LOC_CONF_OFFSET,
      offsetof(rp_http_fastcgi_loc_conf_t, keep_conn),
      NULL },
...
static rp_http_module_t  rp_http_fastcgi_module_ctx = {
    rp_http_fastcgi_add_variables,        /* preconfiguration */
    NULL,                                  /* postconfiguration */

    rp_http_fastcgi_create_main_conf,     /* create main configuration */
    NULL,                                  /* init main configuration */

    NULL,                                  /* create server configuration */
    NULL,                                  /* merge server configuration */

    rp_http_fastcgi_create_loc_conf,      /* create location configuration */
    rp_http_fastcgi_merge_loc_conf        /* merge location configuration */
};


rp_module_t  rp_http_fastcgi_module = {
    RP_MODULE_V1,
    &rp_http_fastcgi_module_ctx,          /* module context */
    rp_http_fastcgi_commands,             /* module directives */
...
static rp_http_fastcgi_request_start_t  rp_http_fastcgi_request_start = {
    { 1,                                               /* version */
      RP_HTTP_FASTCGI_BEGIN_REQUEST,                  /* type */
      0,                                               /* request_id_hi */
      1,                                               /* request_id_lo */
      0,                                               /* content_length_hi */
      sizeof(rp_http_fastcgi_begin_request_t),        /* content_length_lo */
      0,                                               /* padding_length */
      0 },                                             /* reserved */

    { 0,                                               /* role_hi */
      RP_HTTP_FASTCGI_RESPONDER,                      /* role_lo */
      0, /* RP_HTTP_FASTCGI_KEEP_CONN */              /* flags */
      { 0, 0, 0, 0, 0 } },                             /* reserved[5] */

    { 1,                                               /* version */
      RP_HTTP_FASTCGI_PARAMS,                         /* type */
      0,                                               /* request_id_hi */
      1 },                                             /* request_id_lo */
...
static rp_http_variable_t  rp_http_fastcgi_vars[] = {

    { rp_string("fastcgi_script_name"), NULL,
      rp_http_fastcgi_script_name_variable, 0,
      RP_HTTP_VAR_NOCACHEABLE|RP_HTTP_VAR_NOHASH, 0 },

    { rp_string("fastcgi_path_info"), NULL,
      rp_http_fastcgi_path_info_variable, 0,
      RP_HTTP_VAR_NOCACHEABLE|RP_HTTP_VAR_NOHASH, 0 },
...
static rp_str_t  rp_http_fastcgi_hide_headers[] = {
    rp_string("Status"),
...
static rp_keyval_t  rp_http_fastcgi_cache_headers[] = {
...
static rp_path_init_t  rp_http_fastcgi_temp_path = {
    rp_string(RP_HTTP_FASTCGI_TEMP_PATH), { 1, 2, 0 }
};


static rp_int_t
rp_http_fastcgi_handler(rp_http_request_t *r)
{
    rp_int_t                      rc;
    ...
    rp_http_fastcgi_loc_conf_t   *flcf;
#if (RP_HTTP_CACHE)
    rp_http_fastcgi_main_conf_t  *fmcf;
#endif
    ...
    f = rp_pcalloc(r->pool, sizeof(rp_http_fastcgi_ctx_t));
    if (f == NULL) {
        return RP_HTTP_INTERNAL_SERVER_ERROR;
    }

    rp_http_set_ctx(r, f, rp_http_fastcgi_module);

    flcf = rp_http_get_module_loc_conf(r, rp_http_fastcgi_module);

    if (flcf->fastcgi_lengths) {
        if (rp_http_fastcgi_eval(r, flcf) != RP_OK) {
            return RP_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
    ...
    rp_str_set(&u->schema, "fastcgi://");
    u->output.tag = (rp_buf_tag_t) &rp_http_fastcgi_module;

    u->conf = &flcf->upstream;

#if (RP_HTTP_CACHE)
    fmcf = rp_http_get_module_main_conf(r, rp_http_fastcgi_module);

    u->caches = &fmcf->caches;
    u->create_key = rp_http_fastcgi_create_key;
#endif

    u->create_request = rp_http_fastcgi_create_request;
    u->reinit_request = rp_http_fastcgi_reinit_request;
    u->process_header = rp_http_fastcgi_process_header;
    u->abort_request = rp_http_fastcgi_abort_request;
    u->finalize_request = rp_http_fastcgi_finalize_request;
    r->state = 0;
    ...
    u->pipe->input_filter = rp_http_fastcgi_input_filter;
    u->pipe->input_ctx = r;

    u->input_filter_init = rp_http_fastcgi_input_filter_init;
    u->input_filter = rp_http_fastcgi_non_buffered_filter;
...
static rp_int_t
rp_http_fastcgi_eval(rp_http_request_t *r, rp_http_fastcgi_loc_conf_t *flcf)
{
    rp_url_t             url;
    rp_http_upstream_t  *u;
    ...
    if (rp_http_script_run(r, &url.url, flcf->fastcgi_lengths->elts, 0,
...
static rp_int_t
rp_http_fastcgi_create_key(rp_http_request_t *r)
{
    rp_str_t                    *key;
    rp_http_fastcgi_loc_conf_t  *flcf;
    ...
    if (rp_http_complex_value(r, &flcf->cache_key, key) != RP_OK) {
        return RP_ERROR;
    }
...
static rp_int_t
rp_http_fastcgi_create_request(rp_http_request_t *r)
{
    off_t                         file_pos;
    u_char                        ch, *pos, *lowcase_key;
    size_t                        size, len, key_len, val_len, padding,
                                  allocated;
    rp_uint_t                    i, n, next, hash, skip_empty, header_params;
    rp_buf_t                    *b;
    rp_chain_t                  *cl, *body;
    rp_list_part_t              *part;
    ...
    size = sizeof(rp_http_fastcgi_header_t)
           + sizeof(rp_http_fastcgi_begin_request_t)

           + sizeof(rp_http_fastcgi_header_t)  /* RP_HTTP_FASTCGI_PARAMS */
           + len + padding
           + sizeof(rp_http_fastcgi_header_t)  /* RP_HTTP_FASTCGI_PARAMS */

           + sizeof(rp_http_fastcgi_header_t); /* RP_HTTP_FASTCGI_STDIN */
    ...
    rp_http_fastcgi_request_start.br.flags =
        flcf->keep_conn ? RP_HTTP_FASTCGI_KEEP_CONN : 0;

    rp_memcpy(b->pos, &rp_http_fastcgi_request_start,
               sizeof(rp_http_fastcgi_request_start_t));

    h = (rp_http_fastcgi_header_t *)
             (b->pos + sizeof(rp_http_fastcgi_header_t)
                     + sizeof(rp_http_fastcgi_begin_request_t));
    ...
    b->last = b->pos + sizeof(rp_http_fastcgi_header_t)
                     + sizeof(rp_http_fastcgi_begin_request_t)
                     + sizeof(rp_http_fastcgi_header_t);
    ...
    if (padding) {
        rp_memzero(b->last, padding);
        b->last += padding;
    }


    h = (rp_http_fastcgi_header_t *) b->last;
    b->last += sizeof(rp_http_fastcgi_header_t);

    h->version = 1;
    h->type = RP_HTTP_FASTCGI_PARAMS;
    h->request_id_hi = 0;
    h->request_id_lo = 1;
    ...
    if (r->request_body_no_buffering) {
        ...
        u->output.output_filter = rp_http_fastcgi_body_output_filter;
        ...
        while (body) {
            ...
            do {
                ...
                h = (rp_http_fastcgi_header_t *) cl->buf->last;
                cl->buf->last += sizeof(rp_http_fastcgi_header_t);

                h->version = 1;
                h->type = RP_HTTP_FASTCGI_STDIN;
                h->request_id_hi = 0;
                h->request_id_lo = 1;
                h->content_length_hi = (u_char) ((len >> 8) & 0xff);
                h->content_length_lo = (u_char) (len & 0xff);
...
static rp_int_t
rp_http_fastcgi_reinit_request(rp_http_request_t *r)
{
    rp_http_fastcgi_ctx_t  *f;

    f = rp_http_get_module_ctx(r, rp_http_fastcgi_module);

    if (f == NULL) {
        return RP_OK;
    }

    f->state = rp_http_fastcgi_st_version;
    f->fastcgi_stdout = 0;
    f->large_stderr = 0;
...
static rp_int_t
rp_http_fastcgi_body_output_filter(void *data, rp_chain_t *in)
{
    ...
    cl = rp_chain_get_free_buf(r->pool, &f->free);
    if (cl == NULL) {
        return RP_ERROR;
    }
    ...
    b->tag = (rp_buf_tag_t) &rp_http_fastcgi_body_output_filter;
    b->temporary = 1;

    if (b->start == NULL) {
        /* reserve space for maximum possible padding, 7 bytes */

        b->start = rp_palloc(r->pool,
                              sizeof(rp_http_fastcgi_header_t) + 7);
        if (b->start == NULL) {
            return RP_ERROR;
        }
        ...
        b->end = b->start + sizeof(rp_http_fastcgi_header_t) + 7;
    }
    ...
    *ll = cl;
    ...
    while (in) {
        ...
        do {
            tl = rp_chain_get_free_buf(r->pool, &f->free);
            if (tl == NULL) {
                return RP_ERROR;
            }
...
out:
    ...
    rp_chain_update_chains(r->pool, &f->free, &f->busy, &out,
                         (rp_buf_tag_t) &rp_http_fastcgi_body_output_filter);
...
static rp_int_t
rp_http_fastcgi_process_header(rp_http_request_t *r)
{
    u_char                         *p, *msg, *start, *last,
                                   *part_start, *part_end;
    size_t                          size;
    rp_str_t                      *status_line, *pattern;
    rp_int_t                       rc, status;
    rp_buf_t                       buf;
    rp_uint_t                      i;
    rp_table_elt_t                *h;
    ...
    rp_http_fastcgi_loc_conf_t    *flcf;
    rp_http_fastcgi_split_part_t  *part;
    ...
    for ( ;; ) {
        ...
        if (f->state < rp_http_fastcgi_st_data) {

            f->pos = u->buffer.pos;
            f->last = u->buffer.last;

            rc = rp_http_fastcgi_process_record(r, f);
            ...
            if (f->type != RP_HTTP_FASTCGI_STDOUT
                && f->type != RP_HTTP_FASTCGI_STDERR)
            {
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
            ...
            if (f->type == RP_HTTP_FASTCGI_STDOUT && f->length == 0) {
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
        ...
        if (f->state == rp_http_fastcgi_st_padding) {
        ...
        /* f->state == rp_http_fastcgi_st_data */

        if (f->type == RP_HTTP_FASTCGI_STDERR) {
            ...
            if (f->length) {
                ...
                if (u->buffer.pos + f->length <= u->buffer.last) {
                    ...
                    f->length = 0;
                    f->state = rp_http_fastcgi_st_padding;
                ...
                if (flcf->catch_stderr) {
                    ...
                    for (i = 0; i < flcf->catch_stderr->nelts; i++) {
                        if (rp_strnstr(msg, (char *) pattern[i].data,
                                        p - msg)
                            != NULL)
                        {
                            return RP_HTTP_UPSTREAM_INVALID_HEADER;
        ...
        /* f->type == RP_HTTP_FASTCGI_STDOUT */

#if (RP_HTTP_CACHE)

        if (f->large_stderr && r->cache) {
            ssize_t                     len;
            rp_http_fastcgi_header_t  *fh;

            start = u->buffer.start + r->cache->header_start;

            len = u->buffer.pos - start - 2 * sizeof(rp_http_fastcgi_header_t);
            ...
            if (len >= 0) {
                fh = (rp_http_fastcgi_header_t *) start;
                fh->version = 1;
                fh->type = RP_HTTP_FASTCGI_STDERR;
                fh->request_id_hi = 0;
                fh->request_id_lo = 1;
        ...
        for ( ;; ) {
            ...
            rc = rp_http_parse_header_line(r, &u->buffer, 1);
            ...
            if (rc == RP_OK) {
                ...
                if (f->split_parts && f->split_parts->nelts) {
                    ...
                    for (i = 0; i < f->split_parts->nelts; i++) {
                        p = rp_cpymem(p, part[i].start,
                                       part[i].end - part[i].start);
                    }

                    p = rp_cpymem(p, part_start, u->buffer.pos - part_start);

                    buf.last = p;

                    f->split_parts->nelts = 0;

                    rc = rp_http_parse_header_line(r, &buf, 1);
        ...
        if (f->split_parts == NULL) {
            f->split_parts = rp_array_create(r->pool, 1,
                                        sizeof(rp_http_fastcgi_split_part_t));
            if (f->split_parts == NULL) {
                return RP_ERROR;
            }
        }

        part = rp_array_push(f->split_parts);
        if (part == NULL) {
            return RP_ERROR;
        }
...
static rp_int_t
rp_http_fastcgi_input_filter_init(void *data)
{
    rp_http_request_t           *r = data;
    rp_http_fastcgi_loc_conf_t  *flcf;
    ...
    r->upstream->pipe->length = flcf->keep_conn ?
                                (off_t) sizeof(rp_http_fastcgi_header_t) : -1;
...
static rp_int_t
rp_http_fastcgi_input_filter(rp_event_pipe_t *p, rp_buf_t *buf)
{
    u_char                       *m, *msg;
    rp_int_t                     rc;
    rp_buf_t                    *b, **prev;
    ...
    for ( ;; ) {
        if (f->state < rp_http_fastcgi_st_data) {
            ...
            if (f->type == RP_HTTP_FASTCGI_STDOUT && f->length == 0) {
                ...
                rp_log_debug0(RP_LOG_DEBUG_HTTP, p->log, 0,
                               "http fastcgi closed stdout");

                continue;
            }

            if (f->type == RP_HTTP_FASTCGI_END_REQUEST) {
        ...
        if (f->type == RP_HTTP_FASTCGI_STDERR) {
            ...
            if (f->length) {
                ...
                rp_log_error(RP_LOG_ERR, p->log, 0,
    ...
    if (flcf->keep_conn) {
        ...
        } else if (f->state == rp_http_fastcgi_st_padding) {
            p->length = f->padding;
...
static rp_int_t
rp_http_fastcgi_non_buffered_filter(void *data, ssize_t bytes)
{
    u_char                  *m, *msg;
    rp_int_t                rc;
    rp_buf_t               *b, *buf;
...
static rp_int_t
rp_http_fastcgi_process_record(rp_http_request_t *r,
    rp_http_fastcgi_ctx_t *f)
{
    u_char                     ch, *p;
    rp_http_fastcgi_state_e   state;
    ...
    for (p = f->pos; p < f->last; p++) {
        ...
        case rp_http_fastcgi_st_version:
            if (ch != 1) {
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
            ...
            state = rp_http_fastcgi_st_type;
            break;

        case rp_http_fastcgi_st_type:
            switch (ch) {
            case RP_HTTP_FASTCGI_STDOUT:
            case RP_HTTP_FASTCGI_STDERR:
            case RP_HTTP_FASTCGI_END_REQUEST:
                f->type = (rp_uint_t) ch;
                break;
            default:
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
            ...
            state = rp_http_fastcgi_st_request_id_hi;
            break;

        /* we support the single request per connection */

        case rp_http_fastcgi_st_request_id_hi:
            if (ch != 0) {
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
            ...
            state = rp_http_fastcgi_st_request_id_lo;
            break;

        case rp_http_fastcgi_st_request_id_lo:
            if (ch != 1) {
                rp_log_error(RP_LOG_ERR, r->connection->log, 0,
                              "upstream sent unexpected FastCGI "
            ...
            state = rp_http_fastcgi_st_content_length_hi;
            break;

        case rp_http_fastcgi_st_content_length_hi:
            f->length = ch << 8;
            state = rp_http_fastcgi_st_content_length_lo;
            break;

        case rp_http_fastcgi_st_content_length_lo:
            f->length |= (size_t) ch;
            state = rp_http_fastcgi_st_padding_length;
            break;

        case rp_http_fastcgi_st_padding_length:
            f->padding = (size_t) ch;
            state = rp_http_fastcgi_st_reserved;
            break;

        case rp_http_fastcgi_st_reserved:
            state = rp_http_fastcgi_st_data;
        ...
        /* suppress warning */
        case rp_http_fastcgi_st_data:
        case rp_http_fastcgi_st_padding:
            break;
        }
    }
...
static void
rp_http_fastcgi_abort_request(rp_http_request_t *r)
{
    rp_log_debug0(RP_LOG_DEBUG_HTTP, r->connection->log, 0,
...
static void
rp_http_fastcgi_finalize_request(rp_http_request_t *r, rp_int_t rc)
{
    rp_log_debug0(RP_LOG_DEBUG_HTTP, r->connection->log, 0,
...
static rp_int_t
rp_http_fastcgi_add_variables(rp_conf_t *cf)
{
    rp_http_variable_t  *var, *v;

    for (v = rp_http_fastcgi_vars; v->name.len; v++) {
...
static void *
rp_http_fastcgi_create_main_conf(rp_conf_t *cf)
{
    rp_http_fastcgi_main_conf_t  *conf;

    conf = rp_pcalloc(cf->pool, sizeof(rp_http_fastcgi_main_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static void *
rp_http_fastcgi_create_loc_conf(rp_conf_t *cf)
{
    rp_http_fastcgi_loc_conf_t  *conf;

    conf = rp_pcalloc(cf->pool, sizeof(rp_http_fastcgi_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
    ...
    conf->catch_stderr = RP_CONF_UNSET_PTR;

    conf->keep_conn = RP_CONF_UNSET;

    rp_str_set(&conf->upstream.module, "fastcgi");
...
static char *
rp_http_fastcgi_merge_loc_conf(rp_conf_t *cf, void *parent, void *child)
{
    rp_http_fastcgi_loc_conf_t *prev = parent;
    rp_http_fastcgi_loc_conf_t *conf = child;
    ...
    rp_conf_merge_ptr_value(conf->catch_stderr, prev->catch_stderr, NULL);

    rp_conf_merge_value(conf->keep_conn, prev->keep_conn, 0);


    rp_conf_merge_str_value(conf->index, prev->index, "");
    ...
    if (rp_http_upstream_hide_headers_hash(cf, &conf->upstream,
             &prev->upstream, rp_http_fastcgi_hide_headers, &hash)
        != RP_OK)
    {
        return RP_CONF_ERROR;
    }
    ...
    {
        clcf->handler = rp_http_fastcgi_handler;
    }
    ...
    rc = rp_http_fastcgi_init_params(cf, conf, &conf->params, NULL);
    if (rc != RP_OK) {
        return RP_CONF_ERROR;
    }
    ...
    if (conf->upstream.cache) {
        rc = rp_http_fastcgi_init_params(cf, conf, &conf->params_cache,
                                          rp_http_fastcgi_cache_headers);
        if (rc != RP_OK) {
            return RP_CONF_ERROR;
        }
    }
...
static rp_int_t
rp_http_fastcgi_init_params(rp_conf_t *cf, rp_http_fastcgi_loc_conf_t *conf,
    rp_http_fastcgi_params_t *params, rp_keyval_t *default_params)
{
    u_char                       *p;
    size_t                        size;
    uintptr_t                    *code;
...
static rp_int_t
rp_http_fastcgi_script_name_variable(rp_http_request_t *r,
    rp_http_variable_value_t *v, uintptr_t data)
{
    u_char                       *p;
    rp_http_fastcgi_ctx_t       *f;
    ...
    f = rp_http_fastcgi_split(r, flcf);
    ...
    p = rp_copy(v->data, f->script_name.data, f->script_name.len);
    rp_memcpy(p, flcf->index.data, flcf->index.len);
...
static rp_int_t
rp_http_fastcgi_path_info_variable(rp_http_request_t *r,
    rp_http_variable_value_t *v, uintptr_t data)
{
    rp_http_fastcgi_ctx_t       *f;
    rp_http_fastcgi_loc_conf_t  *flcf;
...
static rp_http_fastcgi_ctx_t *
rp_http_fastcgi_split(rp_http_request_t *r, rp_http_fastcgi_loc_conf_t *flcf)
{
    rp_http_fastcgi_ctx_t       *f;
#if (RP_PCRE)
    rp_int_t                     n;
    ...
    n = rp_regex_exec(flcf->split_regex, &r->uri, captures, (1 + 2) * 3);
...
static char *
rp_http_fastcgi_pass(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    rp_http_fastcgi_loc_conf_t *flcf = conf;
    ...
    flcf->upstream.upstream = rp_http_upstream_add(cf, &u, 0);
...
static char *
rp_http_fastcgi_split_path_info(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
#if (RP_PCRE)
    rp_http_fastcgi_loc_conf_t *flcf = conf;
...
static char *
rp_http_fastcgi_store(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    rp_http_fastcgi_loc_conf_t *flcf = conf;
    ...
    if (flcf->upstream.store != RP_CONF_UNSET) {
        return "is duplicate";
    }
...
static char *
rp_http_fastcgi_cache(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    rp_http_fastcgi_loc_conf_t *flcf = conf;
    ...
    if (flcf->upstream.cache != RP_CONF_UNSET) {
        return "is duplicate";
    }
    ...
    if (cv.lengths != NULL) {
        ...
        flcf->upstream.cache_value = rp_palloc(cf->pool,
    ...
    flcf->upstream.cache_zone = rp_shared_memory_add(cf, &value[1], 0,
                                                      &rp_http_fastcgi_module);
...
static char *
rp_http_fastcgi_cache_key(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    rp_http_fastcgi_loc_conf_t *flcf = conf;
...
static char *
rp_http_fastcgi_lowat_check(rp_conf_t *cf, void *post, void *data)
{
#if (RP_FREEBSD)
    ssize_t *np = data;

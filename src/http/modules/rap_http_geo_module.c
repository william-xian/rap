...
#endif
} rap_http_geo_trees_t;


typedef struct {
    rap_http_geo_range_t           **low;
    rap_http_variable_value_t       *default_value;
} rap_http_geo_high_ranges_t;
...
typedef struct {
    ...
    rap_http_variable_value_t       *value;
    size_t                           offset;
} rap_http_geo_variable_value_node_t;


typedef struct {
    rap_http_variable_value_t       *value;
    rap_str_t                       *net;
    rap_http_geo_high_ranges_t       high;
    rap_radix_tree_t                *tree;
#if (RAP_HAVE_INET6)
    rap_radix_tree_t                *tree6;
#endif
...
} rap_http_geo_conf_ctx_t;


typedef struct {
    union {
        rap_http_geo_trees_t         trees;
        rap_http_geo_high_ranges_t   high;
    } u;
...
static rap_int_t rap_http_geo_addr(rap_http_request_t *r,
    rap_http_geo_ctx_t *ctx, rap_addr_t *addr);
static rap_int_t rap_http_geo_real_addr(rap_http_request_t *r,
    rap_http_geo_ctx_t *ctx, rap_addr_t *addr);
static char *rap_http_geo_block(rap_conf_t *cf, rap_command_t *cmd, void *conf);
static char *rap_http_geo(rap_conf_t *cf, rap_command_t *dummy, void *conf);
static char *rap_http_geo_range(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *value);
static char *rap_http_geo_add_range(rap_conf_t *cf,
    rap_http_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end);
static rap_uint_t rap_http_geo_delete_range(rap_conf_t *cf,
    rap_http_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end);
static char *rap_http_geo_cidr(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *value);
static char *rap_http_geo_cidr_add(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_cidr_t *cidr, rap_str_t *value, rap_str_t *net);
static rap_http_variable_value_t *rap_http_geo_value(rap_conf_t *cf,
    rap_http_geo_conf_ctx_t *ctx, rap_str_t *value);
static char *rap_http_geo_add_proxy(rap_conf_t *cf,
    rap_http_geo_conf_ctx_t *ctx, rap_cidr_t *cidr);
static rap_int_t rap_http_geo_cidr_value(rap_conf_t *cf, rap_str_t *net,
    rap_cidr_t *cidr);
static char *rap_http_geo_include(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *name);
static rap_int_t rap_http_geo_include_binary_base(rap_conf_t *cf,
    rap_http_geo_conf_ctx_t *ctx, rap_str_t *name);
static void rap_http_geo_create_binary_base(rap_http_geo_conf_ctx_t *ctx);
static u_char *rap_http_geo_copy_values(u_char *base, u_char *p,
    rap_rbtree_node_t *node, rap_rbtree_node_t *sentinel);


static rap_command_t  rap_http_geo_commands[] = {

    { rap_string("geo"),
      RAP_HTTP_MAIN_CONF|RAP_CONF_BLOCK|RAP_CONF_TAKE12,
      rap_http_geo_block,
...
static rap_http_module_t  rap_http_geo_module_ctx = {
...
rap_module_t  rap_http_geo_module = {
    RAP_MODULE_V1,
    &rap_http_geo_module_ctx,              /* module context */
    rap_http_geo_commands,                 /* module directives */
...
typedef struct {
    ...
    uint32_t  endianness;
    uint32_t  crc32;
} rap_http_geo_header_t;


static rap_http_geo_header_t  rap_http_geo_header = {
...
static rap_int_t
rap_http_geo_cidr_variable(rap_http_request_t *r, rap_http_variable_value_t *v,
    uintptr_t data)
{
    rap_http_geo_ctx_t *ctx = (rap_http_geo_ctx_t *) data;
    ...
    if (rap_http_geo_addr(r, ctx, &addr) != RAP_OK) {
        vv = (rap_http_variable_value_t *)
...
static rap_int_t
rap_http_geo_range_variable(rap_http_request_t *r, rap_http_variable_value_t *v,
    uintptr_t data)
{
    rap_http_geo_ctx_t *ctx = (rap_http_geo_ctx_t *) data;
...
static rap_int_t
rap_http_geo_addr(rap_http_request_t *r, rap_http_geo_ctx_t *ctx,
    rap_addr_t *addr)
{
    rap_array_t  *xfwd;

    if (rap_http_geo_real_addr(r, ctx, addr) != RAP_OK) {
        return RAP_ERROR;
    }
...
static rap_int_t
rap_http_geo_real_addr(rap_http_request_t *r, rap_http_geo_ctx_t *ctx,
    rap_addr_t *addr)
{
    ...
    v = rap_http_get_flushed_variable(r, ctx->index);
    ...
    if (rap_parse_addr(r->pool, addr, v->data, v->len) == RAP_OK) {
        return RAP_OK;
    }
...
static char *
rap_http_geo_block(rap_conf_t *cf, rap_command_t *cmd, void *conf)
{
    char                     *rv;
    size_t                    len;
    ...
    rap_http_geo_ctx_t       *geo;
    rap_http_geo_conf_ctx_t   ctx;
#if (RAP_HAVE_INET6)
    ...
    geo = rap_palloc(cf->pool, sizeof(rap_http_geo_ctx_t));
    if (geo == NULL) {
    ...
    if (cf->args->nelts == 3) {
        ...
        geo->index = rap_http_get_variable_index(cf, &name);
    ...
    rap_memzero(&ctx, sizeof(rap_http_geo_conf_ctx_t));
    ...
    ctx.pool = cf->pool;
    ctx.data_size = sizeof(rap_http_geo_header_t)
                  + sizeof(rap_http_variable_value_t)
                  + 0x10000 * sizeof(rap_http_geo_range_t *);
    ctx.allow_binary_include = 1;

    save = *cf;
    cf->pool = pool;
    cf->ctx = &ctx;
    cf->handler = rap_http_geo;
    ...
    if (ctx.ranges) {
        ...
        if (ctx.high.low && !ctx.binary_include) {
            for (i = 0; i < 0x10000; i++) {
                ...
                len = a->nelts * sizeof(rap_http_geo_range_t);
            ...
            if (ctx.allow_binary_include
                ...
                && ctx.includes == 1)
            {
                rap_http_geo_create_binary_base(&ctx);
            }
        }

        if (ctx.high.default_value == NULL) {
            ctx.high.default_value = &rap_http_variable_null_value;
        }

        geo->u.high = ctx.high;

        var->get_handler = rap_http_geo_range_variable;
        ...
        var->get_handler = rap_http_geo_cidr_variable;
        var->data = (uintptr_t) geo;

        if (rap_radix32tree_insert(ctx.tree, 0, 0,
                                   (uintptr_t) &rap_http_variable_null_value)
            == RAP_ERROR)
        {
            goto failed;
        }
...
static char *
rap_http_geo(rap_conf_t *cf, rap_command_t *dummy, void *conf)
{
    char                     *rv;
    rap_str_t                *value;
    rap_cidr_t                cidr;
    ...
    if (cf->args->nelts == 1) {
        ...
        else if (rap_strcmp(value[0].data, "proxy_recursive") == 0) {
    ...
    if (rap_strcmp(value[0].data, "include") == 0) {
        ...
        rv = rap_http_geo_include(cf, ctx, &value[1]);

        goto done;

    } else if (rap_strcmp(value[0].data, "proxy") == 0) {

        if (rap_http_geo_cidr_value(cf, &value[1], &cidr) != RAP_OK) {
            goto failed;
        }

        rv = rap_http_geo_add_proxy(cf, ctx, &cidr);

        goto done;
    }

    if (ctx->ranges) {
        rv = rap_http_geo_range(cf, ctx, value);

    } else {
        rv = rap_http_geo_cidr(cf, ctx, value);
    }
...
static char *
rap_http_geo_range(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *value)
{
    u_char      *p, *last;
    in_addr_t    start, end;
    ...
    if (rap_strcmp(value[0].data, "default") == 0) {
        ...
        ctx->high.default_value = rap_http_geo_value(cf, ctx, &value[1]);
        if (ctx->high.default_value == NULL) {
            return RAP_CONF_ERROR;
        }
    ...
    if (del) {
        if (rap_http_geo_delete_range(cf, ctx, start, end)) {
            rap_conf_log_error(RAP_LOG_WARN, cf, 0,
    ...
    ctx->value = rap_http_geo_value(cf, ctx, &value[1]);
    ...
    return rap_http_geo_add_range(cf, ctx, start, end);
...
static char *
rap_http_geo_add_range(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        if (a == NULL) {
            a = rap_array_create(ctx->temp_pool, 64,
                                 sizeof(rap_http_geo_range_t));
            if (a == NULL) {
                return RAP_CONF_ERROR;
            }

            ctx->high.low[h] = (rap_http_geo_range_t *) a;
        }
        ...
        while (i) {
            ...
            if (s > (rap_uint_t) range[i].end) {
                ...
                rap_memmove(&range[i + 2], &range[i + 1],
                            (a->nelts - 2 - i) * sizeof(rap_http_geo_range_t));
                ...
                && e < (rap_uint_t) range[i].end)
            {
                ...
                rap_memmove(&range[i + 3], &range[i + 1],
                            (a->nelts - 3 - i) * sizeof(rap_http_geo_range_t));
                ...
                && e < (rap_uint_t) range[i].end)
            {
                ...
                rap_memmove(&range[i + 1], &range[i],
                            (a->nelts - 1 - i) * sizeof(rap_http_geo_range_t));
        ...
        rap_memmove(&range[1], &range[0],
                    (a->nelts - 1) * sizeof(rap_http_geo_range_t));
...
static rap_uint_t
rap_http_geo_delete_range(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    in_addr_t start, in_addr_t end)
{
    in_addr_t              n;
...
static char *
rap_http_geo_cidr(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *value)
{
    char        *rv;
    rap_int_t    rc, del;
    ...
    if (rap_strcmp(value[0].data, "default") == 0) {
        ...
        rv = rap_http_geo_cidr_add(cf, ctx, &cidr, &value[1], &value[0]);
    ...
    if (rap_http_geo_cidr_value(cf, net, &cidr) != RAP_OK) {
        return RAP_CONF_ERROR;
    }
    ...
    return rap_http_geo_cidr_add(cf, ctx, &cidr, &value[1], net);
}


static char *
rap_http_geo_cidr_add(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_cidr_t *cidr, rap_str_t *value, rap_str_t *net)
{
    rap_int_t                   rc;
    rap_http_variable_value_t  *val, *old;

    val = rap_http_geo_value(cf, ctx, value);
...
static rap_http_variable_value_t *
rap_http_geo_value(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *value)
{
    uint32_t                             hash;
    rap_http_variable_value_t           *val;
    rap_http_geo_variable_value_node_t  *gvvn;

    hash = rap_crc32_long(value->data, value->len);

    gvvn = (rap_http_geo_variable_value_node_t *)
    ...
    val = rap_palloc(ctx->pool, sizeof(rap_http_variable_value_t));
    if (val == NULL) {
        return NULL;
    }
    ...
    gvvn = rap_palloc(ctx->temp_pool,
                      sizeof(rap_http_geo_variable_value_node_t));
    if (gvvn == NULL) {
        return NULL;
    }
    ...
    ctx->data_size += rap_align(sizeof(rap_http_variable_value_t) + value->len,
                                sizeof(void *));
...
static char *
rap_http_geo_add_proxy(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_cidr_t *cidr)
{
    rap_cidr_t  *c;

    if (ctx->proxies == NULL) {
        ctx->proxies = rap_array_create(ctx->pool, 4, sizeof(rap_cidr_t));
        if (ctx->proxies == NULL) {
            return RAP_CONF_ERROR;
        }
    }

    c = rap_array_push(ctx->proxies);
    if (c == NULL) {
        return RAP_CONF_ERROR;
    }
...
static rap_int_t
rap_http_geo_cidr_value(rap_conf_t *cf, rap_str_t *net, rap_cidr_t *cidr)
{
    rap_int_t  rc;
...
static char *
rap_http_geo_include(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *name)
{
    char       *rv;
    rap_str_t   file;
    ...
    if (ctx->ranges) {
        ...
        switch (rap_http_geo_include_binary_base(cf, ctx, &file)) {
        case RAP_OK:
            return RAP_CONF_OK;
        case RAP_ERROR:
            return RAP_CONF_ERROR;
...
static rap_int_t
rap_http_geo_include_binary_base(rap_conf_t *cf, rap_http_geo_conf_ctx_t *ctx,
    rap_str_t *name)
{
    u_char                     *base, ch;
    time_t                      mtime;
    ...
    rap_file_t                  file;
    rap_file_info_t             fi;
    rap_http_geo_range_t       *range, **ranges;
    rap_http_geo_header_t      *header;
    rap_http_variable_value_t  *vv;
    ...
    header = (rap_http_geo_header_t *) base;

    if (size < 16 || rap_memcmp(&rap_http_geo_header, header, 12) != 0) {
        rap_conf_log_error(RAP_LOG_WARN, cf, 0,
    ...
    vv = (rap_http_variable_value_t *) (base + sizeof(rap_http_geo_header_t));

    while (vv->data) {
        len = rap_align(sizeof(rap_http_variable_value_t) + vv->len,
                        sizeof(void *));
        rap_crc32_update(&crc32, (u_char *) vv, len);
        vv->data += (size_t) base;
        vv = (rap_http_variable_value_t *) ((u_char *) vv + len);
    }
    rap_crc32_update(&crc32, (u_char *) vv, sizeof(rap_http_variable_value_t));
    vv++;

    ranges = (rap_http_geo_range_t **) vv;

    for (i = 0; i < 0x10000; i++) {
        rap_crc32_update(&crc32, (u_char *) &ranges[i], sizeof(void *));
        if (ranges[i]) {
            ranges[i] = (rap_http_geo_range_t *)
                            ((u_char *) ranges[i] + (size_t) base);
        }
    }

    range = (rap_http_geo_range_t *) &ranges[0x10000];

    while ((u_char *) range < base + size) {
        while (range->value) {
            rap_crc32_update(&crc32, (u_char *) range,
                             sizeof(rap_http_geo_range_t));
            range->value = (rap_http_variable_value_t *)
                               ((u_char *) range->value + (size_t) base);
            range++;
        }
        rap_crc32_update(&crc32, (u_char *) range, sizeof(void *));
        range = (rap_http_geo_range_t *) ((u_char *) range + sizeof(void *));
    }
...
static void
rap_http_geo_create_binary_base(rap_http_geo_conf_ctx_t *ctx)
{
    u_char                              *p;
    uint32_t                             hash;
    rap_str_t                            s;
    ...
    rap_http_geo_range_t                *r, *range, **ranges;
    rap_http_geo_header_t               *header;
    rap_http_geo_variable_value_node_t  *gvvn;
    ...
    p = rap_cpymem(fm.addr, &rap_http_geo_header,
                   sizeof(rap_http_geo_header_t));

    p = rap_http_geo_copy_values(fm.addr, p, ctx->rbtree.root,
                                 ctx->rbtree.sentinel);

    p += sizeof(rap_http_variable_value_t);

    ranges = (rap_http_geo_range_t **) p;

    p += 0x10000 * sizeof(rap_http_geo_range_t *);
    ...
    for (i = 0; i < 0x10000; i++) {
        ...
        range = (rap_http_geo_range_t *) p;
        ranges[i] = (rap_http_geo_range_t *) (p - (u_char *) fm.addr);
        ...
        do {
            ...
            range->value = (rap_http_variable_value_t *) gvvn->offset;
            range->start = r->start;
            range->end = r->end;
            range++;
    ...
    header->crc32 = rap_crc32_long((u_char *) fm.addr
                                       + sizeof(rap_http_geo_header_t),
                                   fm.size - sizeof(rap_http_geo_header_t));
...
static u_char *
rap_http_geo_copy_values(u_char *base, u_char *p, rap_rbtree_node_t *node,
    rap_rbtree_node_t *sentinel)
{
    rap_http_variable_value_t           *vv;
    rap_http_geo_variable_value_node_t  *gvvn;
    ...
    gvvn = (rap_http_geo_variable_value_node_t *) node;
    gvvn->offset = p - base;

    vv = (rap_http_variable_value_t *) p;
    *vv = *gvvn->value;
    p += sizeof(rap_http_variable_value_t);
    ...
    p = rap_http_geo_copy_values(base, p, node->left, sentinel);

    return rap_http_geo_copy_values(base, p, node->right, sentinel);
}

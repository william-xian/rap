...
rap_chain_t *
rap_freebsd_sendfile_chain(rap_connection_t *c, rap_chain_t *in, off_t limit)
{
    int               rc, flags;
    off_t             send, prev_send, sent;
    size_t            file_size;
    ssize_t           n;
    rap_uint_t        eintr, eagain;
    rap_err_t         err;
    rap_buf_t        *file;
    rap_event_t      *wev;
    rap_chain_t      *cl;
...
#if (RAP_HAVE_AIO_SENDFILE && RAP_SUPPRESS_WARN)
    aio = NULL;
    file = NULL;
#endif
    ...
    for ( ;; ) {
        ...
        if (cl && cl->buf->in_file && send < limit) {
            ...
            if (!rap_freebsd_sendfile_nbytes_bug) {
                header.size = 0;
            }

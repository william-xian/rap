...
static rp_connection_t  **event_index;
...
rp_poll_init(rp_cycle_t *cycle, rp_msec_t timer)
{
        ...
        || cycle->old_cycle->connection_n < cycle->connection_n)
    {
        ...
        index = rp_alloc(sizeof(rp_connection_t *) * cycle->connection_n,
                          cycle->log);
        if (index == NULL) {
            return RP_ERROR;
        }
        ...
        if (event_index) {
            rp_memcpy(index, event_index,
                       sizeof(rp_connection_t *) * nevents);
            rp_free(event_index);
        }
...
rp_poll_process_events(rp_cycle_t *cycle, rp_msec_t timer, rp_uint_t flags)
{
    ...
    if (err) {
        rp_log_error(RP_LOG_ALERT, cycle->log, err, "WSAPoll() failed");
        return RP_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
        ...
        if (event_list[i].fd == (rp_socket_t) -1) {
            /*

...
static rap_connection_t  **event_index;
...
rap_poll_init(rap_cycle_t *cycle, rap_msec_t timer)
{
        ...
        || cycle->old_cycle->connection_n < cycle->connection_n)
    {
        ...
        index = rap_alloc(sizeof(rap_connection_t *) * cycle->connection_n,
                          cycle->log);
        if (index == NULL) {
            return RAP_ERROR;
        }
        ...
        if (event_index) {
            rap_memcpy(index, event_index,
                       sizeof(rap_connection_t *) * nevents);
...
rap_poll_process_events(rap_cycle_t *cycle, rap_msec_t timer, rap_uint_t flags)
{
    ...
    if (err) {
        rap_log_error(RAP_LOG_ALERT, cycle->log, err, "WSAPoll() failed");
        return RAP_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
        ...
        if (event_list[i].fd == (rap_socket_t) -1) {
            /*
        ...
        if (c->fd == (rap_socket_t) -1) {
            ...
            if (i == nevents - 1) {
                ...
                event_list[i].fd = (rap_socket_t) -1;
            }

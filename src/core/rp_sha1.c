...
static const u_char *rp_sha1_body(rp_sha1_t *ctx, const u_char *data,
    size_t size);


void
rp_sha1_init(rp_sha1_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
rp_sha1_update(rp_sha1_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        size -= free;
        (void) rp_sha1_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = rp_sha1_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void
rp_sha1_final(u_char result[20], rp_sha1_t *ctx)
{
    size_t  used, free;
...
static const u_char *
rp_sha1_body(rp_sha1_t *ctx, const u_char *data, size_t size)
{

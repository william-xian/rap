...
static const u_char *rap_sha1_body(rap_sha1_t *ctx, const u_char *data,
    size_t size);


void
rap_sha1_init(rap_sha1_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
rap_sha1_update(rap_sha1_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        size -= free;
        (void) rap_sha1_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = rap_sha1_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void
rap_sha1_final(u_char result[20], rap_sha1_t *ctx)
{
    size_t  used, free;
...
static const u_char *
rap_sha1_body(rap_sha1_t *ctx, const u_char *data, size_t size)
{

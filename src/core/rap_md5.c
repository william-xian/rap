...
static const u_char *rap_md5_body(rap_md5_t *ctx, const u_char *data,
    size_t size);


void
rap_md5_init(rap_md5_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
rap_md5_update(rap_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        if (size < free) {
            rap_memcpy(&ctx->buffer[used], data, size);
            return;
        }

        rap_memcpy(&ctx->buffer[used], data, free);
        data = (u_char *) data + free;
        size -= free;
        (void) rap_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = rap_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }

    rap_memcpy(ctx->buffer, data, size);
}


void
rap_md5_final(u_char result[16], rap_md5_t *ctx)
{
    size_t  used, free;
    ...
    if (free < 8) {
        rap_memzero(&ctx->buffer[used], free);
        (void) rap_md5_body(ctx, ctx->buffer, 64);
        used = 0;
        free = 64;
    }

    rap_memzero(&ctx->buffer[used], free - 8);
...
static const u_char *
rap_md5_body(rap_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;

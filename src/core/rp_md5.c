...
static const u_char *rp_md5_body(rp_md5_t *ctx, const u_char *data,
    size_t size);


void
rp_md5_init(rp_md5_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
rp_md5_update(rp_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        if (size < free) {
            rp_memcpy(&ctx->buffer[used], data, size);
            return;
        }

        rp_memcpy(&ctx->buffer[used], data, free);
        data = (u_char *) data + free;
        size -= free;
        (void) rp_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = rp_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }

    rp_memcpy(ctx->buffer, data, size);
}


void
rp_md5_final(u_char result[16], rp_md5_t *ctx)
{
    size_t  used, free;
    ...
    if (free < 8) {
        rp_memzero(&ctx->buffer[used], free);
        (void) rp_md5_body(ctx, ctx->buffer, 64);
        used = 0;
        free = 64;
    }

    rp_memzero(&ctx->buffer[used], free - 8);
...
static const u_char *
rp_md5_body(rp_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;

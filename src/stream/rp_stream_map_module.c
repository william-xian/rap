...
static int rp_libc_cdecl rp_stream_map_cmp_dns_wildcards(const void *one,
    const void *two);
static void *rp_stream_map_create_conf(rp_conf_t *cf);
static char *rp_stream_map_block(rp_conf_t *cf, rp_command_t *cmd,
    void *conf);
static char *rp_stream_map(rp_conf_t *cf, rp_command_t *dummy, void *conf);


static rp_command_t  rp_stream_map_commands[] = {
    ...
    { rp_string("map_hash_max_size"),
      ...
      RP_STREAM_MAIN_CONF_OFFSET,
      offsetof(rp_stream_map_conf_t, hash_max_size),
      NULL },
    ...
    { rp_string("map_hash_bucket_size"),
      ...
      rp_conf_set_num_slot,
      RP_STREAM_MAIN_CONF_OFFSET,
      offsetof(rp_stream_map_conf_t, hash_bucket_size),
      NULL },
...
static rp_stream_module_t  rp_stream_map_module_ctx = {
    NULL,                                  /* preconfiguration */
    NULL,                                  /* postconfiguration */

    rp_stream_map_create_conf,            /* create main configuration */
...
rp_module_t  rp_stream_map_module = {
    RP_MODULE_V1,
    &rp_stream_map_module_ctx,            /* module context */
    rp_stream_map_commands,               /* module directives */
...
static rp_int_t
rp_stream_map_variable(rp_stream_session_t *s, rp_stream_variable_value_t *v,
    uintptr_t data)
{
    rp_stream_map_ctx_t  *map = (rp_stream_map_ctx_t *) data;
    ...
    if (rp_stream_complex_value(s, &map->value, &val) != RP_OK) {
        return RP_ERROR;
    }
    ...
    value = rp_stream_map_find(s, &map->map, &val);
    ...
    if (!value->valid) {
        cv = (rp_stream_complex_value_t *) value->data;

        if (rp_stream_complex_value(s, cv, &str) != RP_OK) {
            return RP_ERROR;
        }
...
static void *
rp_stream_map_create_conf(rp_conf_t *cf)
{
    rp_stream_map_conf_t  *mcf;

    mcf = rp_palloc(cf->pool, sizeof(rp_stream_map_conf_t));
    if (mcf == NULL) {
        return NULL;
    }
...
static char *
rp_stream_map_block(rp_conf_t *cf, rp_command_t *cmd, void *conf)
{
    rp_stream_map_conf_t  *mcf = conf;
    ...
    rp_pool_t                          *pool;
    rp_hash_init_t                      hash;
    rp_stream_map_ctx_t                *map;
    rp_stream_variable_t               *var;
    rp_stream_map_conf_ctx_t            ctx;
    ...
    map = rp_pcalloc(cf->pool, sizeof(rp_stream_map_ctx_t));
    if (map == NULL) {
    ...
    var->get_handler = rp_stream_map_variable;
...
#if (RP_PCRE)
    if (rp_array_init(&ctx.regexes, cf->pool, 2,
                       sizeof(rp_stream_map_regex_t))
        != RP_OK)
    {
        rp_destroy_pool(pool);
        return RP_CONF_ERROR;
    ...
    *cf = save;
    ...
    if (ctx.no_cacheable) {
        var->flags |= RP_STREAM_VAR_NOCACHEABLE;
    }

    map->default_value = ctx.default_value ? ctx.default_value:
                                             &rp_stream_variable_null_value;
    ...
    if (ctx.keys.dns_wc_head.nelts) {
        ...
        rp_qsort(ctx.keys.dns_wc_head.elts,
                  ...
                  sizeof(rp_hash_key_t), rp_stream_map_cmp_dns_wildcards);
...
rp_stream_map_cmp_dns_wildcards(const void *one, const void *two)
{
...
static char *
rp_stream_map(rp_conf_t *cf, rp_command_t *dummy, void *conf)
{
    u_char                              *data;
    size_t                               len;
    rp_int_t                            rv;
    ...
    rp_uint_t                           i, key;
    rp_stream_map_conf_ctx_t           *ctx;
    rp_stream_complex_value_t           cv, *cvp;
    rp_stream_variable_value_t         *var, **vp;
    rp_stream_compile_complex_value_t   ccv;
    ...
    if (vp) {
        for (i = 0; i < ctx->values_hash[key].nelts; i++) {
            ...
            if (vp[i]->valid) {
                ...
                cvp = (rp_stream_complex_value_t *) vp[i]->data;
                data = cvp->value.data;
                len = cvp->value.len;
            }
    ...
    var = rp_palloc(ctx->keys.pool, sizeof(rp_stream_variable_value_t));
    if (var == NULL) {
        return RP_CONF_ERROR;
    }
    ...
    if (cv.lengths != NULL) {
        cvp = rp_palloc(ctx->keys.pool, sizeof(rp_stream_complex_value_t));
        if (cvp == NULL) {
            return RP_CONF_ERROR;
        }
...
found:
    ...
    if (value[0].len && value[0].data[0] == '~') {
        ...
        regex->regex = rp_stream_regex_compile(ctx->cf, &rc);
        if (regex->regex == NULL) {
            return RP_CONF_ERROR;
        }

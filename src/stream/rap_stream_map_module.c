...
} rap_stream_map_conf_ctx_t;


typedef struct {
    rap_stream_map_t              map;
    rap_stream_complex_value_t    value;
    rap_stream_variable_value_t  *default_value;
    rap_uint_t                    hostnames;      /* unsigned  hostnames:1 */
} rap_stream_map_ctx_t;


static int rap_libc_cdecl rap_stream_map_cmp_dns_wildcards(const void *one,
    const void *two);
static void *rap_stream_map_create_conf(rap_conf_t *cf);
static char *rap_stream_map_block(rap_conf_t *cf, rap_command_t *cmd,
    void *conf);
static char *rap_stream_map(rap_conf_t *cf, rap_command_t *dummy, void *conf);


static rap_command_t  rap_stream_map_commands[] = {

    { rap_string("map"),
      RAP_STREAM_MAIN_CONF|RAP_CONF_BLOCK|RAP_CONF_TAKE2,
      rap_stream_map_block,
    ...
    { rap_string("map_hash_max_size"),
      ...
      RAP_STREAM_MAIN_CONF_OFFSET,
      offsetof(rap_stream_map_conf_t, hash_max_size),
      NULL },
    ...
    { rap_string("map_hash_bucket_size"),
      ...
      rap_conf_set_num_slot,
      RAP_STREAM_MAIN_CONF_OFFSET,
      offsetof(rap_stream_map_conf_t, hash_bucket_size),
      NULL },
...
static rap_stream_module_t  rap_stream_map_module_ctx = {
    NULL,                                  /* preconfiguration */
    NULL,                                  /* postconfiguration */

    rap_stream_map_create_conf,            /* create main configuration */
...
rap_module_t  rap_stream_map_module = {
    RAP_MODULE_V1,
    &rap_stream_map_module_ctx,            /* module context */
    rap_stream_map_commands,               /* module directives */
...
static rap_int_t
rap_stream_map_variable(rap_stream_session_t *s, rap_stream_variable_value_t *v,
    uintptr_t data)
{
    rap_stream_map_ctx_t  *map = (rap_stream_map_ctx_t *) data;
    ...
    if (rap_stream_complex_value(s, &map->value, &val) != RAP_OK) {
        return RAP_ERROR;
    }
    ...
    value = rap_stream_map_find(s, &map->map, &val);
    ...
    if (!value->valid) {
        cv = (rap_stream_complex_value_t *) value->data;

        if (rap_stream_complex_value(s, cv, &str) != RAP_OK) {
            return RAP_ERROR;
        }
...
static void *
rap_stream_map_create_conf(rap_conf_t *cf)
{
    rap_stream_map_conf_t  *mcf;

    mcf = rap_palloc(cf->pool, sizeof(rap_stream_map_conf_t));
    if (mcf == NULL) {
        return NULL;
    }
...
static char *
rap_stream_map_block(rap_conf_t *cf, rap_command_t *cmd, void *conf)
{
    rap_stream_map_conf_t  *mcf = conf;
    ...
    rap_stream_map_ctx_t                *map;
    rap_stream_variable_t               *var;
    rap_stream_map_conf_ctx_t            ctx;
    ...
    map = rap_pcalloc(cf->pool, sizeof(rap_stream_map_ctx_t));
    if (map == NULL) {
    ...
    var->get_handler = rap_stream_map_variable;
...
#if (RAP_PCRE)
    if (rap_array_init(&ctx.regexes, cf->pool, 2,
                       sizeof(rap_stream_map_regex_t))
        != RAP_OK)
    {
        rap_destroy_pool(pool);
        return RAP_CONF_ERROR;
    ...
    *cf = save;
    ...
    if (ctx.no_cacheable) {
        var->flags |= RAP_STREAM_VAR_NOCACHEABLE;
    }

    map->default_value = ctx.default_value ? ctx.default_value:
                                             &rap_stream_variable_null_value;
    ...
    if (ctx.keys.dns_wc_head.nelts) {
        ...
        rap_qsort(ctx.keys.dns_wc_head.elts,
                  ...
                  sizeof(rap_hash_key_t), rap_stream_map_cmp_dns_wildcards);
...
rap_stream_map_cmp_dns_wildcards(const void *one, const void *two)
{
...
static char *
rap_stream_map(rap_conf_t *cf, rap_command_t *dummy, void *conf)
{
    u_char                              *data;
    size_t                               len;
    rap_int_t                            rv;
    ...
    rap_uint_t                           i, key;
    rap_stream_map_conf_ctx_t           *ctx;
    rap_stream_complex_value_t           cv, *cvp;
    rap_stream_variable_value_t         *var, **vp;
    rap_stream_compile_complex_value_t   ccv;
    ...
    if (vp) {
        for (i = 0; i < ctx->values_hash[key].nelts; i++) {
            ...
            if (vp[i]->valid) {
                ...
                cvp = (rap_stream_complex_value_t *) vp[i]->data;
                data = cvp->value.data;
                len = cvp->value.len;
            }
    ...
    var = rap_palloc(ctx->keys.pool, sizeof(rap_stream_variable_value_t));
    if (var == NULL) {
        return RAP_CONF_ERROR;
    }
    ...
    if (cv.lengths != NULL) {
        cvp = rap_palloc(ctx->keys.pool, sizeof(rap_stream_complex_value_t));
        if (cvp == NULL) {
            return RAP_CONF_ERROR;
        }
...
found:
    ...
    if (value[0].len && value[0].data[0] == '~') {
        ...
        regex->regex = rap_stream_regex_compile(ctx->cf, &rc);
        if (regex->regex == NULL) {
            return RAP_CONF_ERROR;
        }
